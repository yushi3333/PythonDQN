...
try:
   sys.path.append("C:/Users/a1125/Desktop/CARLA_0.9.11/WindowsNoEditor/PythonAPI/carla/dist/carla-0.9.11-py3.7-win-amd64.egg")
...
class SimEnv(object):
    ...
    def create_actors(self):
        ...
        for _ in range(max_attempts):
            ...
            raise RuntimeError("Failed to spawn vehicle after multiple attempts due to collisions.")
    ...
    def save(self, model, ep):
        if ep % self.save_freq == 0 and ep > self.start_ep:
            ...
            print("Saved model model_ep_{} with average reward =".format(ep), avg_reward)
...
def reward_value(cos_yaw_diff, dist, collision, speed, lambda_1=1, lambda_2=1, lambda_3=15):
    reward = (lambda_1 * cos_yaw_diff) - (lambda_2 * dist) - (lambda_3 * collision)
    
    # good alignment
    if cos_yaw_diff > 0.99 and speed > 5:  
        reward += 0.5
        #print("Aligned well. The angle :", cos_yaw_diff)
    elif cos_yaw_diff <= 0.99 and speed > 5:
        reward -=0.5
        #print(" Not aligned well. The angle :", cos_yaw_diff)   
    
    if dist > 1.0:
        reward -=0.5
        #print("Far from waypoint. The distance :", dist)
    elif dist < 1.0 and speed > 5:
        reward += 0.5
        #print("Close to waypoint. The distance :", dist)
    return reward